...
const { height: warperHeight = 0 } = useSize(warper) ?? { height: 0 }
const isScroll = ref(true)
const activeIndex = ref(0)

const [isScrollHeight, len, _data] = computed(() => {
  const rowH = props.rowHeight + 2
  const _isScroll = props.data.length * rowH > warperHeight && warperHeight > 0
...
const renderList = computed(() => {
  ...
  return _data.value.slice(activeIndex.value, activeIndex.value + len.value)
})
...
}, isScroll.value && isScrollHeight.value)
